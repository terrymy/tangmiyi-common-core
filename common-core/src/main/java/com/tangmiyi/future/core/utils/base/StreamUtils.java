...
/**
 * java8 Streams 的一些简易的封装
 *
 * @see Stream#filter(Predicate) and some on.
 */
public class StreamUtils {
    ...
    public static <F, T> List<T> distinctMap(Collection<F> from, Function<? super F, T> mapper) {
        return from.stream().map(mapper).distinct().collect(Collectors.toList());
    }
    ...
    public static <T> List<T> distinct(List<T> from, Function<? super T, Object> keyExtractor){
        return from.stream().distinct().filter(distinctByKey(keyExtractor)).collect(Collectors.toList());
    }
